...
class TestAccessor:
    ...
    def test_to_pointcloud__equality_and_laziness(self, accessor_dem_path: Path, as_array: bool, lazy: bool) -> None:
        ...
        if as_array:
            ...
            np.testing.assert_array_equal(expected, computed_result)
        else:
            ...
            if lazy:
                ...
                computed_result = result
            assert expected.pointcloud_equal(computed_result)
            assert_frame_equal(expected.ds, computed_result, check_exact=True)
